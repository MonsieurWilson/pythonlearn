...
class ICMPHEADER(Structure):
    ...
    def build(self):
        return struct.pack('!B B H I', self.icmp_type, self.icmp_code, self.icmp_chksum, self.icmp_unused)
...
def decode_icmp(payload, icmp_header_values):
    ...
    _header_values = struct.unpack('!B B H I', icmp_header)
    ...
    icmp_header_values.icmp_unused = _header_values[3]

def build_eth_header(request_eth_header, src_macaddr, dst_macaddr, swap_mac):
    ...
    try:
        ...
        if src_macaddr:
            ...
            src_mac_addr = request_eth_header.get_src_macaddr().split(":")
...
def build_icmp_header(icmp_type, icmp_code, icmp_payload):
    icmp_header = ICMPHEADER(icmp_type, icmp_code, 0, 0)
...
def build_icmp_time_exceeded_packet(outer_interface, request_eth_header, request_ip4_header, payload):
    # Build ICMP header
    icmp_header, icmp_header_pack = \
                        build_icmp_header(11, 0, payload)
    
    # Build IPv4 header
    request_ip4_header.ip_proto = socket.IPPROTO_ICMP
    request_ip4_header.ip_ttl = 64
    ip4_header, ip4_header_pack = \
                        build_ip4_header(request_ip4_header,
                                         getip(outer_interface),
                                         request_ip4_header.get_src_ipaddr(),
                                         icmp_header_pack+payload,
                                         False)

    # Build ethernet header
    eth_header, eth_header_pack = \
                        build_eth_header(request_eth_header,
                                         None,
                                         None,
                                         True)

    return eth_header_pack + ip4_header_pack + icmp_header_pack + payload
...
def print_icmp_header(icmpheader):
    print "ICMP Type: %s Code: %s CheckSum: %s Unused: %s" % (icmpheader.icmp_type, icmpheader.icmp_code, icmpheader.icmp_chksum, icmpheader.icmp_unused)
...
"""Error usage: you must give both the inner interface and outer interface for port forward.
    ...
    request_socket = forward_request_socket = socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.ntohs(0x0800))
    ...
    while True:
        ...
        if int(request_ip4_header.ip_ttl) == 1:
            # send ICMP time exceeded packet to source
            icmp_payload = request_packet[ETHHEADER.header_size: \
                    ETHHEADER.header_size+IP4HEADER.header_size+8]
            icmp_packet = build_icmp_time_exceeded_packet(args.interface,
                                                          request_eth_header,
                                                          request_ip4_header,
                                                          icmp_payload)
            ...
            if args.verbose == "on":
                eth_header = ETHHEADER()
                decode_eth(icmp_packet, eth_header)
                ip_header = IP4HEADER()
                decode_ip(icmp_packet, ip_header)
                icmp_header = ICMPHEADER()
                decode_icmp(icmp_packet, icmp_header)

                print "Send #%d ICMP time exceeded packet:" % packet_count
                print_eth_header(eth_header)
                print_ip4_header(ip_header)
                print_icmp_header(icmp_header)
                print "-" * 100

            forward_request_socket.bind((args.interface, 0))
            forward_request_socket.send(icmp_packet)
